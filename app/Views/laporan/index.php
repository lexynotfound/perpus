...
<section class="content-header">
    ...
    <ol class="breadcrumb nav nav-tabs">
        ...
        <li class="nav-item nav-link active"><a class="nav-link" href="<?= base_url('laporan'); ?>"><i class="fas fa-solid fa-book"></i>&nbsp; Daftar Data Laporan Peminjaman</a></li>
    </ol>
    <!-- Download-->
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800"></h1>
        <a href="<?= base_url('laporan/generatePdf') ?>" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm" id="generateReportBtn">
            <i class="fas fa-download fa-sm text-white-50"></i> Generate Report
        </a>
    </div>
    <!-- Form -->
    <form id="searchForm" method="post" action="<?= base_url('laporan/date') ?>">
        <?= csrf_field() ?>
        <div class="row mb-3">
            <div class="col-md-4">
    ...
    <tbody>
        ...
        foreach ($laporan as $lp) { ?>
            <tr>
                ...
                <td><?= $lp->id_laporan_peminjaman; ?></td>
                <td><?= $lp->judul_laporan; ?></td>
                <td><?= $lp->id_pinjam; ?></td>
                <td><?= $lp->id_buku; ?></td>
                <td><?= $lp->jml_pinjam; ?> Buku Dipinjam</td>
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        html += '<td>' + data[i].id_laporan_peminjaman + '</td>';
                        html += '<td>' + data[i].judul_laporan + '</td>';
                        html += '<td>' + data[i].id_pinjam + '</td>';
                        html += '<td>' + data[i].id_buku + '< Buku Dipinjam /td>';
                        html += '<td>' + data[i].jml_pinjam + '</td>';
