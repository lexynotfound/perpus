...
<section class="content-header">
    ...
    <ol class="breadcrumb nav nav-tabs">
        ...
        <li class="nav-item nav-link active"><a class="nav-link" href="<?= base_url('laporanPengembalian'); ?>"><i class="fas fa-solid fa-book"></i>&nbsp; Daftar Data Laporan Pengembalian</a></li>
    </ol>
    <!-- Download-->
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800"></h1>
        <a href="<?= base_url('laporanpengembalian/generatePdf') ?>" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm" id="generateReportBtn">
            <i class="fas fa-download fa-sm text-white-50"></i> Generate Report
        </a>
    </div>
    <form id="searchForm" method="post" action="<?= base_url('laporanpengembalian/date') ?>">
        <?= csrf_field() ?>
        <div class="row mb-3">
            <div class="col-md-4">
    ...
    <tbody>
        ...
        foreach ($laporanpengembalian as $lp) { ?>
            <tr>
                ...
                <td><?= $lp->id_laporan_pengembalian; ?></td>
                <td><?= $lp->judul_laporan; ?></td>
                <td><?= $lp->id_pengembalian; ?></td>
                <td><?= $lp->status; ?></td>
...
<script>
    $(document).ready(function() {
        ...
        $('#searchForm').on('submit', function(e) {
            ...
            $.ajax({
                url: '<?= base_url('laporanpengembalian/date') ?>',
                type: 'POST',
                data: {
                    startDate: startDate,
                    endDate: endDate,
                ...
                success: function(response) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        html += '<td>' + data[i].id_laporan_pengembalian + '</td>';
                        html += '<td>' + data[i].judul_laporan + '</td>';
                        html += '<td>' + data[i].id_pengembalian + '</td>';
                        html += '<td>' + data[i].status + '< Buku Dipinjam /td>';
                        html += '<td>' + data[i].lama_pinjam + '</td>';
